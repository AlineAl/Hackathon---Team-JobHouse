    ...
    <Link to={routes.lodgingDescription()}>
      <div className="w-72 rounded-lg">
        <div className="relative z-0">
          <img className="w-full bg-cover rounded-lg" src={data.url} alt="" />
        </div>
        <div className="relative border flex flex-col m-auto rounded-lg w-64 p-6 bg-white top-[-30px] z-40">
          <p className="text-xl mb-2">{data.Type}</p>
          <p className="mb-2 text-gray-500 overflow-hidden text-ellipsis h-[72px]">
            {data.description}
          </p>
          <hr />
...
const LodgingPagePage = () => {
  const { data } = useQuery(LODGINGS_QUERY)
  return (
    <>
      ...
      <section className="my-12 mx-6">
        ...
        <div className="flex justify-between">
          <div className="flex flex-wrap justify-around w-[60%]">
            {data &&
              data.lodgings.map((item) => {
                return <LodgingCard key={item.id} data={item} />
              })}
          </div>
